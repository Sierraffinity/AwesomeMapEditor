...
void QHexEdit::ensureVisible()
{
    if (_cursorPosition < (_bPosFirst * 2))
        verticalScrollBar()->setValue((int)(_cursorPosition / 2 / lineWidth()));
    if (_cursorPosition > ((_bPosFirst + (_rowsShown - 1)*lineWidth()) * 2))
        verticalScrollBar()->setValue((int)(_cursorPosition / 2 / lineWidth()) - _rowsShown + 1);
...
void QHexEdit::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        setCursorPosition(_cursorPosition | (2 * lineWidth() -1));
        resetSelection(_cursorPosition);
    }
    if (event->matches(QKeySequence::MoveToStartOfLine))
    {
        setCursorPosition(_cursorPosition - (_cursorPosition % (2 * lineWidth())));
        resetSelection(_cursorPosition);
    }
    if (event->matches(QKeySequence::MoveToPreviousLine))
    {
        setCursorPosition(_cursorPosition - (2 * lineWidth()));
        resetSelection(_cursorPosition);
    }
    ...
    {
        setCursorPosition(_cursorPosition + (((_rowsShown - 1) * 2 * lineWidth())));
        resetSelection(_cursorPosition);
    }
    ...
    {
        qint64 pos = _cursorPosition - (_cursorPosition % (2 * lineWidth())) + (2 * lineWidth());
        setCursorPosition(pos);
        setSelection(pos);
    }
    if (event->matches(QKeySequence::SelectStartOfLine))
    {
        qint64 pos = _cursorPosition - (_cursorPosition % (2 * lineWidth()));
        setCursorPosition(pos);
        setSelection(pos);
    }
    if (event->matches(QKeySequence::SelectPreviousLine))
    {
        qint64 pos = _cursorPosition - (2 * lineWidth());
        setCursorPosition(pos);
        setSelection(pos);
    }
    ...
    {
        qint64 pos = _cursorPosition + (((viewport()->height() / _pxCharHeight) - 1) * 2 * lineWidth());
        setCursorPosition(pos);
        setSelection(pos);
    }
...
void QHexEdit::paintEvent(QPaintEvent *event)
{
    ...
    if (event->rect() != _cursorRect)
    {
        ...
        {
            QString address;
            for (int row=0, pxPosY = _pxCharHeight; row <= (_dataShown.size() / lineWidth()); row++, pxPosY +=_pxCharHeight)
            {
                address = QString("%1").arg(_bPosFirst + row*lineWidth() + _addressOffset, _addrDigits, 16, QChar('0')).toUpper();
        ...
        for (int row = 0, pxPosY = pxPosStartY; row <= _rowsShown; row++, pxPosY +=_pxCharHeight)
        {
            ...
            qint64 bPosLine = row * lineWidth();
            for (int colIdx = 0; ((bPosLine + colIdx) < _dataShown.size() && (colIdx < lineWidth())); colIdx++)
            {
...
void QHexEdit::adjust()
{
    ...
    _pxPosAsciiX = _pxPosHexX + (lineWidth() * 3 - 1) * _pxCharWidth + _pxGapHexAscii;

    // set horizontalScrollBar()
    int pxWidth = _pxPosAsciiX;
    if (_asciiArea)
        pxWidth += lineWidth()*_pxCharWidth;
    ...
    int lineCount = (int)(_chunks->size() / (qint64)lineWidth())/* + 1*/;
    verticalScrollBar()->setRange(0, lineCount - _rowsShown);
    verticalScrollBar()->setPageStep(_rowsShown);

    int value = verticalScrollBar()->value();
    _bPosFirst = (qint64)value * lineWidth();
    _bPosLast = _bPosFirst + (qint64)(_rowsShown * lineWidth()) - 1;
...
{
    _dataShown = _chunks->data(_bPosFirst, _bPosLast - _bPosFirst + lineWidth() + 1, &_markedShown);
